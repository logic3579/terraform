...
| **iam** | `aws_iam_role` + `aws_iam_instance_profile` + `aws_iam_role_policy_attachment` + `aws_iam_role_policy`, `aws_iam_user` + `aws_iam_user_policy_attachment` + `aws_iam_user_policy` | One role + one instance profile per `ec2_instance_profiles` entry (same name); Lambda execution roles created separately; `iam_users` provisions standalone IAM users with managed-policy attachments + inline policies. Console passwords and programmatic access keys are intentionally not managed here — create them out-of-band to keep credentials out of state. Outputs `ec2_instance_profile_names`, `ec2_role_arns`, `lambda_role_arns`, `iam_user_arns`, `iam_user_names` maps for root wiring. |
